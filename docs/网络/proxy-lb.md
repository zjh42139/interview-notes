...
**正向代理代理客户端（翻墙、企业内网）、反向代理代理服务端（Nginx、负载均衡）。Nginx 是前端最常打交道的反向代理——静态资源服务、API 转发、负载均衡、HTTPS 终结（TLS termination）。Vite proxy 在开发环境做的事就是简化版反向代理。**
  ...
  server: {
    proxy: {
      '/api': {
        ...
        changeOrigin: true,  // 修改请求头中的 Host 为目标地址，否则后端 Host 校验会失败
      },
    },
  },
};
...
```nginx
# HTTPS 终结（TLS termination）+ HTTP2 + 静态资源 + API 代理 + 缓存策略
server {
...
- 2026-07-18：Phase 3 事实审计——"HTTPS 终端"更正为"HTTPS 终结（TLS termination）"；修正 changeOrigin 注释（改的是 Host 头，不是 origin）
