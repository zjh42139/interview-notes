...
4. **密钥交换（ECDHE）**：现代 HTTPS 几乎都使用 ECDHE（椭圆曲线临时 Diffie-Hellman）密钥交换。服务端先发送签名后的 DH 参数（ServerKeyExchange），客户端回复自己的 DH 参数（ClientKeyExchange）。双方用各自的私钥 + 对方的公钥独立计算出相同的会话密钥——传输过程中私钥从未离开各自手中。ECDHE 的核心优势是**前向安全性**：即使服务器的长期私钥未来泄露，历史会话也无法解密，因为每次握手都生成临时的 DH 密钥对。
   > 补充：RSA 密钥交换（客户端生成 pre-master 用公钥加密发过去）已过时，TLS 1.3 将其移除。
