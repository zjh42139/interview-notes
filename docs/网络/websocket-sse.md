...
### 短轮询 vs 长轮询 — WebSocket/SSE 之前的方案

面试常把四种"服务端如何把消息送到前端"的方案放在一起对比：

- **短轮询（Polling）**：前端 `setInterval` 每隔几秒发一次普通 HTTP 请求问"有新消息吗"。实现最简单、兼容性最好，但实时性差（最坏延迟 = 轮询间隔），且大部分请求是拿不到新数据的空轮询。
- **长轮询（Long Polling）**：前端发请求后服务端**不立即响应**，把连接挂起（hold）直到有新消息或超时才返回；前端收到响应后立刻发起下一次请求。实时性接近推送，但服务端要维持大量挂起连接，且每条消息都带完整的 HTTP 头开销。典型场景：扫码登录的状态查询、早年的 Web 聊天室。

| 方案 | 实时性 | 开销 | 方向 | 实现成本 |
|------|--------|------|------|---------|
| 短轮询 | 差（秒级延迟） | 高频无效请求 | 客户端拉 | 最低 |
| 长轮询 | 较好 | 挂起连接多、每条消息重发头部 | 客户端拉（伪推送） | 低 |
| SSE | 好 | 一条 HTTP 长连接 | 服务端推（单向） | 低 |
| WebSocket | 最好 | 一条独立协议长连接 | 双向全双工 | 中（需心跳/重连） |

选型逻辑：一次性/低频场景用轮询兜底（如扫码登录），单向推送用 SSE，双向高频交互用 WebSocket。Socket.IO 的"自动降级"降到的就是 HTTP 长轮询。
...
SSE 走标准 HTTP 端口，不需要 Nginx 做额外的 WebSocket 代理配置，在公司内网环境下代理/网关不会阻断。但要注意关闭代理的响应缓冲（`proxy_buffering off` 或响应头 `X-Accel-Buffering: no`），否则事件会被 Nginx 攒在缓冲区里迟迟不下发。
...
export function useDashboardWS() {
  ...
  let ws = null, timer = null, attempts = 0
  function connect() {
...
- 2026-07-18：Phase 3 事实审计——新增"短轮询 vs 长轮询"小节（四方案对比）；修复示例中未声明的 `attempts`；补 SSE 经 Nginx 需关 `proxy_buffering`；`Last-Event-ID` 大小写
