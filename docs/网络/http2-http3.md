...
- **0-RTT 连接恢复**：如果你之前和某个服务器建立过 QUIC 连接，第二次连接时可以直接在第一个包里带上加密的应用数据，不需要握手等待。对比 TCP + TLS 1.3 最快也要 1-RTT。**但 0-RTT 存在重放攻击风险**：攻击者可以捕获 0-RTT 数据包并重新发送，服务端可能重复执行同一个请求（如重复扣款）。因此 0-RTT 只应用于幂等请求（GET），敏感操作（POST/PUT/DELETE）应禁用或要求服务端实现防重放机制。
