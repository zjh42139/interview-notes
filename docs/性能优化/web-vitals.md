...
// impact fraction: 元素移动前后两帧占据区域的并集，占视口的比例
// distance fraction: 元素移动距离占视口的比例
//
// 例如：一个占视口 50% 的图片向下移动了视口高度的 25%
// impact fraction = 50% + 25% = 0.75（移动前后覆盖的总区域）
// CLS = 0.75 × 0.25 = 0.1875 （已超标）
```
...
// 目标 <= 800ms
//
// FCP (First Contentful Paint)：首次内容绘制，首个文本/图片内容上屏
// 目标 <= 1.8s
```
...
- **CrUX（Chrome 用户体验报告）**：真实用户数据，28 天滚动窗口，能反映线上真实表现
...
| 面试官问 | 下一问大概率是 |
|----------|-------------|
| "Web Vitals 核心指标有哪些" | 追问 LCP/INP/CLS 各自衡量什么、良好阈值，以及 INP 为何取代 FID（FID 已于 2024 年 3 月退役） |
...
- 2026-07-18：Phase 3 事实审计修正（CrUX 28 天窗口、信号表不再把 FID 列为现行指标、FCP 定义、补 Metric 类型导入）
