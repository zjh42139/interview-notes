...
Lighthouse 模拟中端设备（默认 Moto G Power + Slow 4G 节流 + 4 倍 CPU 降速），产出 4 个维度的评分：

```ts
// Lighthouse 核心指标（0-100 分）
// Performance：FCP / SI / LCP / TBT / CLS 综合加权
// Accessibility：ARIA 标签、对比度、焦点顺序
// Best Practices：HTTPS、安全策略、废弃 API
// SEO：meta 标签、结构化数据、可爬取性
// 注意：PWA 维度已在 Lighthouse 12（2024 年 5 月）移除，别再说"五大维度"
...
// 8. 关注具名构造函数（你的业务类/组件）和 Detached DOM 节点
//    (closure)/(array) 等带括号的是引擎内部分类，@ 后的数字只是对象 ID
```
...
```ts
// 瀑布图（Waterfall）分析：
// 灰色（Queueing / Stalled）→ 请求排队与阻塞（HTTP/1.1 下同域最多 6 个 TCP 连接，HTTP/2 无此限制）
// 深绿（DNS Lookup）→ DNS 解析
// 橙色（Initial connection）→ TCP 握手（其中 SSL 握手段显示为紫色）
// 绿色（Waiting / TTFB）→ 服务器处理时间（后端瓶颈）
...
- 2026-07-18：Phase 3 事实审计修正（Lighthouse 12 移除 PWA 维度、模拟设备更正为 Moto G Power + Slow 4G、Heap Snapshot 构造函数识别方式、Network 瀑布图颜色与 HTTP/1.1 并发限制表述）
