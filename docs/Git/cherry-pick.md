...
注意：C' 的 **内容** 和 C 一样，但是一个全新的 commit，hash 完全不同，parent 是 G 而不是 B。message 默认保留原 message，后面会加上 `(cherry picked from commit <原hash>)`。
...
1. **重复提交**：忘记录来源 `-x`，同一个修复出现在两个分支上，将来 merge 时 Git 可能无法识别这是同一个变更，再次产生冲突
