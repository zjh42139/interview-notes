...
`WeakMap` 的 key 只能是对��，且 key 的引用是**弱引用**——这意味着：如果对该对象的其他引用都已消失，即使它还在 WeakMap 里作为 key，GC 也会回收它。
...
- **"WeakMap 可以用来存任意键值对"**：不能。WeakMap 的 key **必须是对象**，且不可迭代。它的设计初衷是为对象附加元数据，而不是做普通的键值存储。
