...
**Web Storage 是浏览器提供的客户端数据存储体系，包括 Cookie（4KB，每次请求自动携带）、LocalStorage（5-10MB，持久化，同源共享）、SessionStorage（5-10MB，会话级，标签页隔离）和 IndexedDB（大容量，受磁盘配额限制，异步，支持事务和索引），各有不同的容量、生命周期和安全模型，需要根据数据敏感度和访问模式选择。**
...
| **作用域** | 按 Domain + Path 隔离（不区分端口） | 同源，所有标签页共享 | 同源，**标签页隔离** | 同源，所有标签页共享 |
...
**核心**：设置 `SameSite=Lax`（或 `Strict`）后，攻击者无法通过 `<form action="...">` 或 AJAX 跨站提交带 Cookie 的请求，大幅收窄了 CSRF 的攻击面。但注意两点：Lax 下跨站 GET 顶级导航仍会携带 Cookie（所以状态变更操作绝不能用 GET）；实践中 SameSite 应与 CSRF Token 配合做纵深防御，而不是唯一防线。
