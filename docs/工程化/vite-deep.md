...
但 esbuild 也有代价：不支持 AST 级别的自定义插件（只支持简单的 onLoad/onResolve 钩子），面向旧目标（如 ES5）的语法降级不完整，ESM/CJS 互操作有边界 bug。所以 Vite 的**打包（bundle）**环节生产上回归 Rollup——但注意 esbuild 并没有在生产构建中缺席：TS/JSX 的单文件转译和默认的产物压缩（`minify: "esbuild"`）仍由 esbuild 完成。
...
2. **开发正常、生产报错** -- 开发时 CJS 由 esbuild 预构建转换，生产打包时由 `@rollup/plugin-commonjs` 转换，两者边界行为不同。`optimizeDeps.include` 只影响开发预构建；生产侧的 CJS 问题要通过 `build.commonjsOptions` 调整
