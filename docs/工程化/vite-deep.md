...
**Vite 利用浏览器原生 ESM 实现"按需编译"（不改不编译）。Vite 7 及以前用 Go 编写的 esbuild 做依赖预构建 + Rollup 生产打包；Vite 8+ 起 Rolldown（Rust 实现）统一开发和生产打包，Oxc 负责转译和压缩，esbuild 和 Rollup 不再作为 Vite 依赖——三阶段分层设计，各取所长。**
...
但 esbuild 也有代价：不支持 AST 级别的自定义插件（只支持简单的 onLoad/onResolve 钩子），面向旧目标（如 ES5）的语法降级不完整，ESM/CJS 互操作有边界 bug。所以 Vite 7 及以前的**打包（bundle）**环节生产上回归 Rollup——Vite 8+ 起则统一为 Rolldown（Rust 实现的 Rollup 继任者，10-30x 性能提升），转译和压缩由 Oxc 接手，esbuild 和 Rollup 不再作为 Vite 依赖安装。
