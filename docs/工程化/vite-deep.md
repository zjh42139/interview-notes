...
**Vite 利用浏览器原生 ESM 实现"按需编译"（不改不编译），用 Go 编写的 esbuild 做依赖预构建消除 node_modules 的性能瓶颈。Vite 7 及以前生产环境回归 Rollup 保证产物质量；Vite 8+ 起 Rolldown（Rust 实现）统一开发和生产打包——三阶段分层设计，各取所长。**
...
| AST | 手写解析器，全流程只遍历 AST 约 3 次 | 多工具链反复 parse -> transform -> generate |
| 内存 | 一次分配、复用，GC 压力小 | 大量临时对象，GC 停顿 |
| 速度 | 0.3s | 15-30s（同等规模） |

关键点：esbuild **并非不生成 AST**，而是把对 AST 的完整遍历压缩到约 3 次（解析+作用域建立、符号绑定+语法转换、代码生成），且 AST 数据结构为内存紧凑设计。对比之下，Babel 生态往往是多个工具串联，每个工具各自 parse -> transform -> generate，代码在字符串和 AST 之间反复转换，产生大量中间对象。

但 esbuild 也有代价：不支持 AST 级别的自定义插件（只支持简单的 onLoad/onResolve 钩子），面向旧目标（如 ES5）的语法降级不完整，ESM/CJS 互操作有边界 bug。所以 Vite 7 及以前的**打包（bundle）**环节生产上回归 Rollup——Vite 8+ 起则统一为 Rolldown（Rust 实现的 Rollup 继任者，10-30x 性能提升）。注意 esbuild 并没有在生产构建中缺席：TS/JSX 的单文件转译和默认的产物压缩（`minify: "esbuild"`）仍由 esbuild 完成。
...
| 生产构建 | Rolldown（Vite 8+）/ Rollup（Vite 7-），代码分割、Tree Shaking 成熟 | Webpack 5（持久化缓存 + 增量） |
...
Vite 7 及以前的策略是"开发求快、生产求稳"——开发用 esbuild（快就行了，边界 bug 可以容忍），生产用 Rollup（稳 + 质量优先）。Vite 8+ 起 Rolldown（Rust 实现）统一了这两个角色——它继承了 Rollup 的 ESM-first 设计哲学和 Tree Shaking 能力，同时用 Rust 实现带来 10-30x 的性能提升，99%+ 现有 Rollup 插件兼容。
...
export default defineConfig({
  ...
  // 生产构建分包策略
  // Vite 8+ 统一为 Rolldown 配置，向后兼容 rollupOptions
  build: {
    rollupOptions: {
      output: {
        manualChunks: {
...
2. **开发正常、生产报错** -- 开发时 CJS 由 esbuild 预构建转换，生产打包时（Vite 7- 用 `@rollup/plugin-commonjs`，Vite 8+ 用 Rolldown 内置的 CJS 处理）转换，两者边界行为不同。`optimizeDeps.include` 只影响开发预构建；生产侧的 CJS 问题要通过 `build.commonjsOptions` 调整
...
> "第一层是架构差异：Vite 开发时不打包，利用浏览器原生 ESM 按需编译，Webpack 必须先全量打包再启动 dev server。第二层是语言优势：esbuild 用 Go 编写，比 JS 写的 Babel 快 10-100 倍——编译为机器码、多线程并行、全流程只遍历 AST 约 3 次。第三层是 HMR 效率：基于 ESM 的精准失效，改哪个模块只重新请求哪个模块，而 Webpack HMR 需要增量编译 + 生成 chunk 差异。但生产打包 Vite 8+ 用 Rolldown（Rust 实现的 Rollup 继任者，10-30x 性能提升），Vite 7 及以前用 Rollup——因为 esbuild 在代码分割和 Tree Shaking 上不如 Rollup/Rolldown 成熟（esbuild 仍负责生产的 TS 转译和压缩）。"
...
- 2026-07-21：Vite 8 Rolldown 更新——生产构建从 Rollup 切换为 Rolldown（Rust 实现），更新一句话总结、esbuild 局限分析、对比表、生产策略、面试信号、易错点
