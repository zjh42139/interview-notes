...
**Vite 是基于浏览器原生 ESM 的开发服务器，开发环境用 esbuild 做依赖预构建实现毫秒级冷启动。Vite 8 起，生产环境用 Rolldown（Rust 实现的 Rollup 继任者）统一开发和生产打包，替代旧的 esbuild(dev)+Rollup(prod) 双工具架构。**
...
**Vite 8 起（2026-03）**：Rolldown 成为默认唯一打包器。Rolldown 是 Rollup 作者 Rich Harris 参与开发的 Rust 重写版，继承了 Rollup 的 ESM-first 设计哲学和 Tree Shaking 能力，同时用 Rust 实现带来 10-30x 的构建性能提升（实际案例：108s→5.2s、46s→6s），99%+ 现有 Rollup 插件兼容。esbuild 仍负责预构建/压缩——这个没变。
