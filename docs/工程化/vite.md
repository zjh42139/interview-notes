...
**Vite 是当下 Vue3 生态的标配构建工具，面试必问。** 理解 Vite 不仅仅是会用 `vite.config.ts`，更要理解它"为什么快"——ESM 开发服务器 + esbuild 预构建 + Rolldown 生产打包（Vite 8+，Rust 实现，替代旧的 esbuild+Rollup 双架构）。

## 一句话总结

**Vite 是基于浏览器原生 ESM 的开发服务器。Vite 7 及以前：开发环境用 esbuild 做依赖预构建实现毫秒级冷启动，生产用 Rollup。Vite 8 起（2026-03）：Rolldown（Rust 实现的 Rollup 继任者）统一开发和生产打包，Oxc 负责转译压缩，替代旧的 esbuild+Rollup 双工具架构。**
...
graph LR
    ...
    C -->|是| D[返回预构建缓存<br/>Vite7-: esbuild<br/>Vite8+: Rolldown]
...
3. **esbuild 预构建依赖（Vite 7 及以前）**：node_modules 里的第三方包（如 Element Plus）先用 esbuild 转成 ESM 并合并，避免浏览器发几百个请求。Vite 8+ 预构建由 Rolldown 统一处理。

### 预构建（esbuild → Rolldown 演进）

**Vite 7 及以前**，预构建由 esbuild 负责——这是 Vite 快的关键。esbuild 用 Go 语言编写，比 Babel/tsc 快 10-100 倍：
...
### 生产构建：Rolldown（Vite 8+）

**Vite 7 及以前**：生产用 Rollup。为什么生产不用 esbuild？esbuild 的产物优化能力不如 Rollup（代码分割、Tree Shaking 颗粒度、插件生态）。所以 Vite 当时采用"开发 esbuild + 生产 Rollup"的双工具策略。

**Vite 8 起（2026-03）**：Rolldown 成为默认唯一打包器，**统一替代了旧的 esbuild(预构建) + Rollup(生产打包) 双工具架构**。Rolldown 是 Rollup 作者 Rich Harris 参与开发的 Rust 重写版，继承了 Rollup 的 ESM-first 设计哲学和 Tree Shaking 能力，同时用 Rust 实现带来 10-30x 的构建性能提升（实际案例：108s→5.2s、46s→6s），99%+ 现有 Rollup 插件兼容。配套工具链为 Rolldown(打包) → Oxc(解析/转换/压缩) + Lightning CSS——esbuild 和 Rollup **不再作为 Vite 依赖安装**。
...
| 生产构建 | Rolldown（Vite 8+）/ Rollup（Vite 7-） | Webpack 5 |
| 学习成本 | 低（开箱即用） | 高（loader/plugin 概念多） |

**为什么开发用 esbuild、生产用 Rollup/Rolldown？** esbuild 快但不稳定（某些 ESM/CJS 互操作有 bug），且代码分割和 Tree Shaking 不如 Rollup 成熟。Vite 7 及以前采用"开发求快（esbuild），生产求稳（Rollup）"的策略。Vite 8 起 Rolldown 统一开发和生产——它用 Rust 实现，继承了 Rollup 的 ESM-first 设计哲学和 Tree Shaking 能力，同时性能提升 10-30x。
...
| 面试官问 | 下一问大概率是 |
|----------|-------------|
| "Vite 为什么快" | 追问原生 ESM dev server + esbuild 预构建 + Rolldown（Rust）生产打包 |
...
flowchart TD
  ...
  C --> D["Vite 生产构建<br/>Rolldown（Rust）打包"]
...
- 2026-07-21：Vite 8 Rolldown 更新——生产构建从 Rollup 切换为 Rolldown（Rust 实现），更新一句话总结、生产构建章节、对比表、面试信号表、Mermaid 图
