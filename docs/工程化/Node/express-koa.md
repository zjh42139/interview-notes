...
| 错误处理 | 4 参数错误中间件 + `next(err)` | `try/catch` + `app.on('error')` 事件 |
| 核心体量 | 较大（内置功能多） | 极简（核心只有中间件引擎 + ctx 封装，两者都有第三方依赖，并非零依赖） |
...
- **`ctx.body` 不设置时默认 404**：Koa 中如果整条中间件链都没有设置 `ctx.body` 也没设置 `ctx.status`，响应是 404 Not Found（不是空 200）；多次设置 `ctx.body` 不会报错但只有最后一次生效
- **混淆 `ctx.req` 和 `ctx.request`**：`ctx.req` 是 Node 原生对象，`ctx.request` 是 Koa 封装对象。应该用 `ctx.request`（或直接 `ctx.body` / `ctx.query` 这些快捷属性）
- **以为 `ctx.throw()` 不捕获会崩溃进程**：不会。中间件抛出的错误（含 `ctx.throw`）会被 Koa 内置的 `ctx.onerror` 兜底——返回对应状态码的默认文案并触发 `app.on('error')` 事件。但默认响应对用户不友好，生产中应在洋葱最外层统一 try/catch 做错误包装和上报
