...
**`<Transition>` 为单个元素的进入/离开添加过渡动画（六个 class + JavaScript 钩子）；`<TransitionGroup>` 为列表元素的增删移动添加动画（额外加了 v-move 类实现 FLIP）。默认由 CSS 驱动——Vue 只在合适的时机添加/移除 class；也可以用 JavaScript 钩子接管（配合 GSAP 等）。**
...
1. **改 `name` 不等于重播动画** —— `:name` 可以动态绑定（官方支持的"动态过渡"用法），但它只影响**下一次**进入/离开使用的 class 前缀；过渡只在元素插入/移除时触发。想强制重新播放动画，用修改 key 强制重新挂载
