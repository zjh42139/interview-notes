...
**单个缓存组件实例的内存占用因组件树规模和缓存数量而异（可通过 DevTools Memory 面板实测）**——任何"一个页面 XX MB"的具体数字都只是估算，实际取决于数据量和子组件复杂度。可以确定的方向是：`max` 越大，持有的实例引用链越多，内存压力越大。
...
- 2026-07-18：移除无依据的内存占用具体数字（5-20MB / 250MB-1GB），改为"因组件树规模和缓存数量而异，可通过 DevTools Memory 面板实测"
