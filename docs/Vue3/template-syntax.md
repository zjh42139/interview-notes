...
```
**为什么不能用 index 当 key？** 列表开头插入/删除元素，所有后续元素的 index 全变——Diff 算法会认为所有节点都变了，全部重新渲染而不是复用。只有静态列表（从不增删改排序）可以用 index。
...
❌ **用 index 当 key** —— 面试必问。列表有增删时 index 全乱——Diff 全量重渲染。
