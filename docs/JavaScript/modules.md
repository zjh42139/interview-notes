...
// CJS 加载 ESM：老版本不能直接 require
// require('./modern.mjs'); // ❌ Node 22.12 之前报 ERR_REQUIRE_ESM
// 传统方案：动态 import()（返回 Promise；CJS 没有顶层 await，需包在 async 函数里）
async function load() {
  const esmModule = await import('./modern.mjs');
}
// 新变化：Node 22.12+ / 23+ 起 require() 可以直接加载"同步的"ESM
// （模块图里不含顶层 await 才行）——面试提到这一点是加分项
```
