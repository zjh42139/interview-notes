...
iframe 是浏览器原生提供的**页面嵌套**能力，每个 iframe 拥有独立的 `window`、`document`、CSSOM、JS 执行环境。优点是无与伦比的隔离性（JS/CSS/DOM 天然隔离），缺点是六大硬伤：**URL 不同步、UI 不同步、通信靠 postMessage、性能开销大、SEO 不友好、全局上下文割裂**。它的定位不是通用微前端方案，而是**特定场景的最优解**——富文本编辑器实时预览、第三方页面安全嵌入、跨域独立页面加载。
...
#### 缺陷三：通信靠 postMessage —— 异步、字符串

iframe 和父页面通信的唯一方式是 `postMessage`，这是一个**异步、字符串序列化、全局广播**的 API。
...
2. **字符串**：复杂对象（包含函数、Symbol、Date 等）不能被 `postMessage` 完整序列化
3. **安全**：不校验 `event.origin` 可能导致 XSS 攻击

#### 缺陷四：性能开销 —— 独立渲染进程

每个 iframe 在 Chromium 中是独立的 `SiteInstance`，有独立的渲染进程。10 个 iframe = 10 个独立渲染进程，每个都有独立的内存分配、DOM 解析、样式计算。
...
2. **再说缺点**："但不适合做通用微前端方案，因为 6 个硬伤：URL 不同步、UI 不同步、postMessage 通信复杂、性能开销大、SEO 不友好、全局上下文割裂"
...
- [XSS 安全](../../浏览器/安全/xss) — postMessage 的安全校验与 XSS 防护
