...
同一组件在不同路由间复用（如 `/user/1` → `/user/2`），如果不设置 `key`，KeepAlive 会按**组件类型名**缓存，导致切换参数后复用了缓存的旧数据。用 `fullPath` 作为 key，确保："同组件不同参数 = 不同缓存实例"。
...
因为 `include`/`exclude` 在组件被 render 时**通过 `vnode.type.name` 或 `vnode.type.__name` 来匹配**。如果你没有在 `defineOptions` 里声明 `name`，Vue 会默认使用文件名作为组件名 —— 但这个行为在 `script setup` 下也不总是可靠的（生产构建后可能被压缩）。**显式设置 `name` 是最稳妥的做法。**

```ts
// ✅ 显式声明
defineOptions({ name: 'UserList' })

// ❌ 依赖隐式推断，打包后可能变成随机的 minified name
```
