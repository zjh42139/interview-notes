...
function lazyLoadWithRetry(path: string, retries = 2) {
  return async () => {
    let lastError: Error
    for (let i = 0; i <= retries; i++) {
      try {
        ...
        if (i < retries) {
          console.warn(`Chunk 加载失败，第 ${i + 1} 次重试: ${path}`)
        }
      }
    }
    // 全部失败后显示错误页面
...
const routes = [
  {
    path: '/dashboard',
    // 带重试的懒加载
    component: lazyLoadWithRetry('@/views/dashboard/index.vue')
  }
]
...
// 公共 loader 工厂：生成带错误处理的路由懒加载函数
function viewLoader(name: string, retries = 2) {
  return () => import(
    /* @vite-ignore */
    `@/views/${name}.vue`
  ).catch((err) => {
    console.error(`[Router] 加载组件失败: ${name}`, err)
  ...
  children: [
    {
      path: 'list',
      name: 'UserList',
      component: viewLoader('user/List'),
      meta: { title: '用户列表', keepAlive: true }
    },
    {
      path: 'detail/:id',
      name: 'UserDetail',
      component: viewLoader('user/Detail'),
      meta: { title: '用户详情', activeMenu: '/user/list' }
    },
    {
      path: 'form/:id?',
      name: 'UserForm',
      component: viewLoader('user/Form'),
...
**❌ `webpackChunkName` 带 `-` 和空格等特殊字符**
chunk 名称尽量用 `a-z`、`0-9`、`-` 的字符集，不要带 `/`（会被当作路径）。
