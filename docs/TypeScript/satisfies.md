...
`satisfies` 是 TypeScript "类型安全"工具链中的一环，把它放到更大的类型断言体系中对比会更清晰：

- **`as` 断言**：不改变运行时行为，只是告诉 TypeScript "相信我，这就是这个类型"。编译器跳过检查，风险由开发者承担。`(data as User).name` 如果 data 不是 User，运行时直接炸。
- **类型声明 `: Type`**：编译时检查赋值兼容性。`const u: User = data` 会在编译期验证 data 是否符合 User 的结构。比 `as` 安全，但会丢失更精确的推导类型（如字面量类型被扩宽）。
- **非空断言 `!`**：`data!.name` 告诉 TS "我确定 data 不是 null/undefined"。本质是 `as` 的特化版本——绕过空值检查。在严格模式下应尽量避免，用类型守卫或可选链替代。
- **双重断言 `as unknown as T`**：先断言为 `unknown`（万物之源），再断言为目标类型。这**绕过所有类型检查**，是类型系统的逃生舱，不是常规手段。只有在明确知道 TS 推断有误且无法用类型守卫修正时（如第三方库类型定义错误）才作为最后手段使用。

`as`、`satisfies`、`: Type` 三者的定位：`as` 是"强扭"（我说是什么就是什么）、`: Type` 是"体检"（编译期验证符合才放行）、`satisfies` 是"体检但不改变身份"（验证但不改写推导结果）。
...
- 2026-07-18：新增「类型断言体系」小节——as 断言、类型声明、非空断言、双重断言的对比与危险性
