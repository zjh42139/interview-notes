...
| 策略 | 适用场景 | 行为 |
|------|---------|------|
| **bundler** | Vite/Webpack | 支持 `exports` 字段（同 node16），但不要求文件扩展名——打包器会处理路径 |
| **node16/nodenext** | Node.js ESM | 完整模拟 Node.js ESM+CJS interop，要求扩展名 |
| **node** | 旧 Node CJS | 模拟 require() 查找——不支持 exports 字段 |

**面试关键**：Vite 项目用 `bundler`——不要求 `.js`/`.ts` 扩展名，打包器会处理。Node 原生 ESM 项目用 `node16`——相对导入必须写完整扩展名，并按 `package.json` 的 exports 字段解析包入口。选错了——TS 的行为和运行时不一致，编译能过运行报 `module not found`。
...
3. 单文件转译需要——Babel/esbuild 逐文件转译时看不到跨文件类型信息；`isolatedModules` 下重导出类型必须用 `export type`，更严格的 `verbatimModuleSyntax`（TS 5.0）要求所有仅类型导入都显式写 `import type`
...
❌ **漏配 `esModuleInterop`** —— 默认值是 false（`module` 为 node16/nodenext/preserve 时默认 true），`tsc --init` 生成的模板会显式开启；手写精简配置时容易漏掉——`import React from 'react'` 报错。
...
- 2026-07-18：事实审计——修正 bundler 策略描述（支持 exports 字段）、node16 与 exports 的关系、isolatedModules/verbatimModuleSyntax 表述、esModuleInterop 与 tsc --init 的关系
