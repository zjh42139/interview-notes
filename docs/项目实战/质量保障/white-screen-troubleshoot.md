...
1. **被 loading 状态误导——真白屏 vs 正常加载**：页面正在渲染 loading 动画时 DOM 根节点也是空的——白屏检测脚本会误报。解决：加一个 3 秒延迟——页面加载 3 秒后 `#app` 仍然为空才判定为白屏。同时检查 `document.readyState`——`loading` 状态时不触发检测。

2. **JS 报错被全局捕获后页面依然白屏但不自知**：`errorHandler` 捕获了错误但没有做任何 UI 反馈——用户看到的是永远不消失的 loading 或者空白页。每个 catch 分支必须有 UI 反馈——展示 error 组件或跳转错误页。

3. **资源 404 导致的白屏被当成 JS 错误**：`/chunk-xxx.js` 404 时浏览器报的是 `Script error`——无具体信息（跨域资源）。但这实际上是资源加载失败而非 JS 执行错误。排查时先看 Network 面板的红色请求，再看 Console——顺序不能反。

4. **路由懒加载失败的白屏**：`() => import('/views/xxx.vue')` 失败时 Vue Router 只报 `Loading chunk failed`——页面停留在当前状态或跳转到一个空白的路由。解决：在路由配置中加 `error` 处理——加载失败时重试或跳转错误页。

5. **CDN 不可用导致所有资源 404**：静态资源部署在 CDN 上，CDN 挂了 → `index.html` 能加载但所有 JS/CSS 404 → 白屏。应急方案：DNS 层面切到备用 CDN 或源站。预防方案：`index.html` 内联一个探针脚本，检测 CDN 资源是否正常加载。
...
| "路由懒加载失败导致白屏怎么处理" | 追问 chunk 加载失败后的重试策略和错误路由 |
| "CDN 挂了怎么兜底" | 备用域名 + 内联探针检测 + DNS 切换预案 |
