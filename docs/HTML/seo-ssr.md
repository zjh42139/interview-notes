...
```ts
// Nuxt 中通过 routeRules 的 swr（stale-while-revalidate）实现类似 ISR 效果
// nuxt.config.ts
export default defineNuxtConfig({
  routeRules: {
    // 该路由渲染结果在服务端缓存，1 小时内直接复用
    '/news/**': { swr: 3600 },
  },
})
// 第一个用户触发页面生成，后续用户看到缓存版本
// 过期后的第一个请求触发后台重新生成，该用户暂时还是看到旧版本
```
...
流式 SSR 的最大优势是 **首字节更早到达（TTFB 降低）、FCP 明显提前**——服务器不必等整页渲染完才开始发送，浏览器可以边接收边渲染。
