...
对题数 ≥16 的 7 个题库加 `##` 分组标题（对齐 HTML 题库已有结构），组内按 ⭐ 降序排列并重新编号：

- **JavaScript（32 题）**: 基础语法与类型 8 / 原型与面向对象 5 / 异步编程 6 / 手写实现 7 / 数组与数据结构 6
- **Vue3（22 题）**: 响应式系统 6 / 渲染机制 5 / 组件机制 6 / 进阶与生态 5
- **CSS（22 题）**: 盒模型与布局 8 / 定位与层叠 4 / 选择器与优先级 2 / 动画与过渡 2 / 现代特性与实践 6
- **TypeScript（24 题）**: 类型基础 7 / 泛型与高级类型 6 / 工具类型与实战 7 / 工程实践 4
- **浏览器（20 题）**: 页面渲染与加载 5 / 安全与存储 7 / 引擎与性能 5 / 工具与扩展 3
- **网络（17 题）**: 协议基础 6 / 安全与缓存 5 / 应用与实践 6
- **工程化（16 题）**: 构建工具 7 / 模块化与优化 4 / 质量与部署 5

题目内容零改动（git diff 逐行核验）；文内 Q 互引同步更新（Vue3 事件总线→Q12）；跨库引用修复 2 处（CICD→浏览器 Q5→Q8、http-methods 更新记录去 Q 号）。题数 ≤11 的模块（Git/Pinia/VueRouter/CICD/前端架构）维持平铺。
