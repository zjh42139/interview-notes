...
.table-container {
  width: 100%;
  overflow-x: auto;
  -webkit-overflow-scrolling: touch; /* 仅 iOS 12 及以下需要，iOS 13+ 已默认惯性滚动 */
}
```
...
- ❌ **`width: 100vw` 没考虑滚动条宽度**：Windows 滚动条占 17px，`100vw` 包含滚动条宽度，导致横向溢出。✅ 用 `width: 100%`，或 `scrollbar-gutter: stable` 预留滚动条空间（`100dvw` 同样包含滚动条宽度，它解决的是移动端地址栏伸缩，不解决这个问题）。
...
- 2026-07-18：事实审计——修正 100dvw 滚动条误导、标注 -webkit-overflow-scrolling 仅老 iOS 需要
