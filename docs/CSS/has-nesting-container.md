...
**和 SCSS 的区别**：SCSS 的 `&` 是编译期字符串拼接——`&__title` 能拼出 `.card__title`；CSS 嵌套的 `&` 是真正的选择器引用——不能做字符串拼接，且嵌套按 `:is()` 语义解析（外层是选择器列表时，权重取列表中最高者）。
...
- 2026-07-18：事实审计——修正"SCSS & vs CSS 嵌套 &"的区别描述（字符串拼接 vs 选择器引用 + :is() 语义）
