...
Pinia 通过 `defineStore` 定义 Store，支持 **Setup Store**（函数式，推荐）和 **Options Store**（配置式，类 Vuex）两种语法。Store 的 id 必须全局唯一，命名遵循 `useXxxStore` 规范；组件内在 `setup()` 中调用实例化，组件外使用需确保 pinia 已安装激活。
...
export const useOrderStore = defineStore('order', () => {
  const userStore = useUserStore()  // action/getter 内部可以
...
为什么推荐在 setup 中调用？`useXxxStore()` 需要拿到 `pinia` 实例：在 setup 上下文中通过 `inject` 获取；在组件外则回退到「当前激活的 pinia」（`app.use(pinia)` 或 `setActivePinia()` 之后才存在）。所以在 `app.use(pinia)` 之前的模块顶层调用会报 `getActivePinia() was called but there was no active Pinia`，组件外使用的正确姿势见[进阶：组件外使用](./advanced.md)。

## 深度拓展

### 追问1：$reset() 在 Setup Store 中如何实现？

Options Store 的 `$reset()` 会调用 `state()` 函数重新获取初始状态并替换。Setup Store 没有 `state()` 函数，直接调用 `$reset()` 会抛错（Pinia 提示 setup 语法未实现 `$reset`），需要手动实现：
...
- 2026-07-18：事实审计：修正「必须在 setup 中调用」的绝对化表述与获取 pinia 实例的机制说明，补充 Setup Store $reset 抛错，修 typo
- 2026-07-06：初始创建
